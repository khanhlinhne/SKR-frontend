// Admin Courses Components — Barrel export

// Old components (some might still be used or kept for reference/Expert view)
export { default as CourseStatsGrid } from './CourseStatsGrid';
export { default as ChapterList } from './ChapterList';
export { default as CoursePricingCard } from './CoursePricingCard';
export { default as CourseCreatorCard } from './CourseCreatorCard';
export { default as CourseReviewsList } from './CourseReviewsList';
export { default as CourseInfoCard } from './CourseInfoCard';

// New Business/Admin focused components
export { default as RevenueChart } from './RevenueChart';
export { default as EnrollmentTrend } from './EnrollmentTrend';
export { default as RecentOrdersTable } from './RecentOrdersTable';
export { default as RevenueBreakdown } from './RevenueBreakdown';
export { default as BusinessKPICards } from './BusinessKPICards';
export { default as CourseSummaryCard } from './CourseSummaryCard';
export { default as ExpertAssignmentCard } from './ExpertAssignmentCard';
export { default as CourseEditModal } from './CourseEditModal';

export { containerVariants, cardVariants, statusConfig, mockCourseDetail } from './constants';
