...
export default function AddFlashcardCardModal({
    ...
    const handleImageUpload = async (cardId, side, file) => {
        ...
        if (file.size > MAX_FLASHCARD_IMAGE_SIZE_BYTES) {
            setFormError('?nh quá l?n. Vui lòng ch?n ?nh du?i 5MB.');
            return;
        }
        ...
        try {
            ...
            if (!imageUrl) {
                throw new Error('Cú chua nh?n du?c URL ?nh t? máy ch?.');
            }
            updateCard(cardId, imageField, imageUrl);
        } catch (error) {
            setFormError(error?.response?.data?.message || error?.message || 'Cú chua t?i du?c ?nh lên. B?n th? l?i nhé.');
        } finally {
    ...
    const buildPayload = () => {
        ...
        if (halfFilled.length > 0) {
            setFormError('M?i th? c?n d? c? m?t tru?c và m?t sau. B?n hãy di?n d? ho?c xóa dòng còn dang d?.');
            return null;
        }
        ...
        if (validCards.length === 0) {
            setFormError('Hãy nh?p ít nh?t m?t th? hoàn ch?nh tru?c khi luu.');
            return null;
        }

        if (Object.values(uploadingSlots).some(Boolean)) {
            setFormError('?nh v?n dang t?i lên. Cú c?n b?n ch? hoàn t?t tru?c khi luu.');
            return null;
        }
    ...
    const handleGenerateWithAI = async () => {
        ...
        if (!sourceText) {
            setFormError('Hãy nh?p ch? d?, do?n ghi chú ho?c dàn n?i dung bài h?c d? Gemini t?o flashcard.');
            return;
        }

        if (Object.values(uploadingSlots).some(Boolean)) {
            setFormError('?nh v?n dang t?i lên. Hãy d?i xong r?i m?i dùng Gemini d? tránh l?ch d? li?u.');
            return;
        }
        ...
        try {
            ...
            setFormError(error?.message || 'Gemini chua t?o du?c n?i dung flashcard. B?n th? l?i v?i mô t? c? th? hon.');
        } finally {
                ...
                <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                            <h3 className="text-xl font-black text-base-content">{isEditMode ? 'Ch?nh s?a th? flashcard' : 'Thêm th? flashcard'}</h3>
                            <p className="mt-1 text-sm text-base-content/55">
                                {isEditMode
                                    ? 'C?p nh?t l?i n?i dung m?t tru?c, m?t sau và hình ?nh c?a th? này.'
                                    : 'Nh?p nhanh m?t tru?c và m?t sau d? t?o nhi?u th? liên ti?p cho bài h?c.'}
                            </p>
                            {setTitle && (
                    ...
                    {!isEditMode && (
                        <div className="rounded-[28px] border border-indigo-100 bg-gradient-to-r from-indigo-50/90 via-violet-50/60 to-white p-4 shadow-sm">
                            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                <div className="min-w-0">
                                    ...
                                    <p className="mt-1 text-xs text-base-content/55">
                                        Dán ch? d?, do?n ghi chú ho?c n?i dung bài h?c. Gemini s? t?o b? th? ti?ng Vi?t d? b?n ch?nh s?a và luu ngay trong modal này.
                                    </p>
                                </div>
                                <div className="flex items-center gap-2 self-start">
                                    ...
                                    >
                                        ...
                                        {aiGenerating ? 'Ðang t?o...' : 'T?o b?ng Gemini'}
                                    </button>
                                </div>
                            </div>
                            <textarea
                                ...
                                placeholder="VD: T?o 5 th? flashcard v? JSX trong React, t?p trung vào khái ni?m, cú pháp và khi nào nên dùng..."
                            ...
                            <p className="mt-2 text-[11px] font-medium text-base-content/45">
                                N?u bên du?i dã có th? nh?p tay, k?t qu? m?i s? du?c thêm ti?p vào cu?i danh sách thay vì ghi dè.
                            </p>
                        </div>
                    )}
                                ...
                                <div className="grid gap-4 xl:grid-cols-2">
                                    ...
                                    <div className="rounded-2xl border border-violet-100 bg-white p-4 shadow-sm">
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Gi?i thích ng?n g?n, ghi nh? chính ho?c dáp án..."
                    ...
                    <div className="modal-action items-center justify-between">
                        <div className="text-xs text-base-content/45">
                            {isEditMode
                                ...
                                : nextOrder != null
                                    ...
                                    : 'B?n có th? thêm nhi?u th? liên ti?p'}
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            >
                                ...
                                {isEditMode ? 'Luu thay d?i' : 'Luu các th?'}
                            </button>
                        </div>
                    </div>
                </form>
