...
function buildManualQuestionFromSource(question, index) {
    ...
    return {
        ...
        options: normalizedType === 'true_false'
            ...
            : (sanitizedOptions.length > 0 ? sanitizedOptions : createEmptyManualQuestion(Date.now() + index).options),
    };
}

function extractInitialQuestions(initialTest) {
    const questions = findQuestionsInPayload(initialTest);
