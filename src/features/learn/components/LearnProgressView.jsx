...
const LEARN_COLOR_STYLES = {
    blue: { text: 'text-blue-500', bg: 'bg-blue-500', softBg: 'bg-blue-500/10', softSurface: 'bg-blue-500/5', softBorder: 'border-blue-500/10', textStrong: 'text-blue-600' },
    emerald: { text: 'text-emerald-500', bg: 'bg-emerald-500', softBg: 'bg-emerald-500/10', softSurface: 'bg-emerald-500/5', softBorder: 'border-emerald-500/10', textStrong: 'text-emerald-600' },
    amber: { text: 'text-amber-500', bg: 'bg-amber-500', softBg: 'bg-amber-500/10', softSurface: 'bg-amber-500/5', softBorder: 'border-amber-500/10', textStrong: 'text-amber-600' },
    violet: { text: 'text-violet-500', bg: 'bg-violet-500', softBg: 'bg-violet-500/10', softSurface: 'bg-violet-500/5', softBorder: 'border-violet-500/10', textStrong: 'text-violet-600' },
};
                                    ...
                                    <div key={i}>
                                        <div className="flex items-center justify-between text-[11px] mb-1">
                                            <span className="flex items-center gap-1.5 text-base-content/60 font-medium">
                                                <ItemIcon className={`w-3 h-3 ${(LEARN_COLOR_STYLES[item.color] || LEARN_COLOR_STYLES.blue).text}`} />
                                                {item.label}
                                            </span>
                                        ...
                                        <div className="w-full h-1.5 rounded-full bg-base-200 overflow-hidden">
                                            <motion.div
                                                ...
                                                className={`h-full rounded-full ${(LEARN_COLOR_STYLES[item.color] || LEARN_COLOR_STYLES.blue).bg}`}
                                            />
                                        </div>
                                    </div>
                                );
...
function StatCard({ variants, icon: Icon, label, done, total, color, gradient }) {
    ...
    return (
        <motion.div variants={variants} className="bg-base-100 rounded-2xl border border-base-300 p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
                <div className={`w-7 h-7 rounded-lg ${(LEARN_COLOR_STYLES[color] || LEARN_COLOR_STYLES.blue).softBg} flex items-center justify-center`}>
                    <Icon className={`w-3.5 h-3.5 ${(LEARN_COLOR_STYLES[color] || LEARN_COLOR_STYLES.blue).text}`} />
                </div>
...
function InsightItem({ emoji, title, text, color }) {
    return (
        <div className={`p-3 rounded-xl ${(LEARN_COLOR_STYLES[color] || LEARN_COLOR_STYLES.blue).softSurface} border ${(LEARN_COLOR_STYLES[color] || LEARN_COLOR_STYLES.blue).softBorder}`}>
            <p className={`text-[11px] font-bold ${(LEARN_COLOR_STYLES[color] || LEARN_COLOR_STYLES.blue).textStrong} mb-0.5`}>{emoji} {title}</p>
