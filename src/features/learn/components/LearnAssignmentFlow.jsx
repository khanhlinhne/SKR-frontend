...
export default function LearnAssignmentFlow({
    ...
    const handleSubmit = async () => {
        if (!answerText.trim()) {
            setError('Hãy nhập câu trả lời trước khi nộp bài.');
            return;
        }
        ...
        try {
            ...
            setError(submitError?.message || 'Không thể nộp bài tập lúc này.');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    if (!assignment) {
        return (
            <div className="rounded-3xl border border-dashed border-base-300 bg-base-100 p-10 text-center shadow-xl">
                <p className="text-lg font-black text-base-content">Bài tập này chưa được soạn đề bài</p>
                <p className="mt-2 text-sm text-base-content/50">
                    Expert cần thêm đề bài và rubric trước khi học viên có thể nộp bài.
                </p>
            </div>
        );
    }
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                        ...
                        <p className="mt-2 max-w-3xl text-sm leading-6 text-base-content/65">
                            {assignment.description
                                || 'Hoàn thành bài tập này và nộp câu trả lời để AI chấm theo rubric đã được expert đặt trước.'}
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="rounded-full bg-rose-500/10 px-3 py-1 text-xs font-bold text-rose-600">
                            {`${assignment.rubricCriteria?.length || 0} tiêu chí`}
                        </span>
                        <span className="rounded-full bg-base-200 px-3 py-1 text-xs font-bold text-base-content/60">
                            {`${assignment.maxScore || 100} điểm`}
                        </span>
            ...
            <div className="grid gap-6 px-6 py-6 lg:grid-cols-[1.05fr,0.95fr]">
                <div className="space-y-5">
                    <div className="rounded-3xl border border-base-300 bg-base-200/35 p-5">
                        <p className="text-[11px] font-black uppercase tracking-[0.16em] text-base-content/40">Yêu cầu bài làm</p>
                        <p className="mt-3 text-sm leading-7 text-base-content/75">
                            {assignment.instructions || assignment.submissionFormat || 'Trả lời bằng văn bản.'}
                        </p>
                    ...
                    {viewMode === 'result' && submission ? (
                        <div className="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 via-white to-emerald-50 p-5 shadow-sm">
                            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    ...
                                    <p className="mt-3 text-sm leading-6 text-base-content/70">
                                        {submission.summary || 'Bài nộp đã được lưu và chấm điểm theo rubric.'}
                                    </p>
                                </div>
                            ...
                            <div className="mt-5 grid gap-4 lg:grid-cols-2">
                                <div className="rounded-2xl border border-emerald-100 bg-white p-4">
                                    <p className="text-[11px] font-black uppercase tracking-[0.16em] text-base-content/40">Điểm theo rubric</p>
                                        ...
                                        <div className="mt-3 space-y-2">
                                            {(submission.strengths || []).length > 0 ? (
                                                submission.strengths.map((item, index) => (
                                                    <div key={index} className="rounded-xl bg-emerald-500/8 px-3 py-2 text-sm text-base-content/75">
                                                        {item}
                                                    </div>
                                                ))
                                            ) : (
                                        ...
                                        <div className="mt-3 space-y-2">
                                            {(submission.improvements || []).length > 0 ? (
                                                submission.improvements.map((item, index) => (
                                                    <div key={index} className="rounded-xl bg-amber-500/8 px-3 py-2 text-sm text-base-content/75">
                                                        {item}
                                                    </div>
                                                ))
                                            ) : (
                        ...
                        <div className="rounded-3xl border border-base-300 bg-base-100 p-5 shadow-sm">
                            <div className="mb-3 flex items-center justify-between gap-3">
                                <div>
                                    <p className="text-sm font-black text-base-content">Câu trả lời của bạn</p>
                                    <p className="mt-1 text-xs text-base-content/55">
                                        Nộp bài bằng văn bản. Bạn có thể nộp lại để AI chấm bản cập nhật mới.
                                    </p>
                                </div>
                                {isCompleted && (
                            ...
                            <textarea
                                ...
                                placeholder="Nhập câu trả lời, phân tích, đoạn code hoặc lập luận của bạn..."
                            ...
                            <div className="mt-3 flex items-center justify-between gap-3 text-xs text-base-content/45">
                                <span>{`${answerText.trim().length} ký tự`}</span>
                        ...
                        <div className="mt-4 space-y-3">
                            {viewMode === 'result' && submission ? (
                                ...
                                <button
                                    ...
                                    className="btn w-full rounded-2xl border-none bg-gradient-to-r from-rose-500 to-orange-500 font-bold text-white shadow-lg shadow-rose-500/20 disabled:opacity-70"
                                >
                                    {submitting ? (
                                    ...
                                    {submitting ? 'Đang nộp và chấm bài...' : 'Nộp assignment'}
                                </button>
                            )}
